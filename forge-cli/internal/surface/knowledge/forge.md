...
- **MCP tools**: discovered from MCP servers declared in `forge.yaml`'s
  ...
  **Standalone login without a `forge.yaml`** (for non-forge agents — Strands,
  Claude): `forge mcp login <name> --url <url>` takes the connection inline and
  skips forge.yaml (`--client-id`/`--scopes`/`--authorize-url`/`--token-url`/
...
The proxy's **upstream** (what it forwards to — the Anthropic API by default, or an org gateway like Kong/Bedrock) resolves via `resolveOptimizerUpstream` with precedence **managed settings `optimizer.upstream` > `--upstream` > `$FORGE_OPTIMIZER_UPSTREAM` > user settings `optimizer.upstream` > `$ANTHROPIC_BASE_URL` chaining > Anthropic default**. Like the enable flag, `optimizer.upstream` is read from **trusted layers only** (the checked-in project layer is excluded — it redirects Claude Code's traffic + Authorization header), a **managed** value overrides even `--upstream`, and the resolved value is validated as an http(s) URL at startup. `forge optimizer start` resolves it once and forwards it to the detached child via `--upstream`; `status` shows it. See `docs/reference/cli-reference.md` (`forge optimizer`).
