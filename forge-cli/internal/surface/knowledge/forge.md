...
**Native OIDC (`models.gateway(s).oidc`, #455 slice 2):** an alternative to `api_key_helper` where forge does the OAuth flow itself (`forge-cli/runtime/gateway_oidc.go`). `grant: client_credentials` mints headlessly (`token_url`/`issuer` + `client_id` + `client_secret_env` + `scopes`) — the overlay acquires/re-mints on expiry; `grant: auth_code` runs an interactive browser login (auth-code+PKCE via `oauth.Flow`, fixed loopback `localhost:1455/auth/callback`) cached with its refresh token, which the overlay refreshes silently (never a browser mid-run — else it tells you to run `forge auth login`). Endpoints via `authorize_url`/`token_url` or `issuer`'s `/.well-known/openid-configuration`; the client secret is env-only. Same trusted-layer resolution. **Guardrail:** the OIDC flow refuses `api.anthropic.com` (that API is x-api-key, not OAuth). `forge auth login|logout|status` cover OIDC gateways too. Cache key `GatewayOIDCCredKey(oidc)`. Device-code grant deferred.
