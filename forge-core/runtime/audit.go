...
type AuditEvent struct {
	...
	// PrevHash carries the sha256 of the previous event's canonical
	// JSON bytes (as they were written to the sink stream, WITHOUT
	// the trailing newline). Together with the per-emit re-computation
	// this forms a hash chain over the stream — any post-hoc alteration
	// to a prior event breaks the chain at that point.
	//
	...
	// The field is written on EVERY event (no omitempty) so a trimmed
	// or absent value is itself a tampering signal.
	//
	// See docs/security/audit-tamper-evidence.md (governance R5, #212).
...
type AuditLogger struct {
	...
	// canonical JSON bytes. Each Emit stamps event.PrevHash = lastHash
	// then updates lastHash to sha256(current event). The very first
	// Emit sees lastHash == "" and writes AuditChainGenesis so downstream
	// verifiers have a well-defined start value.
	//
	// Guarded by mu; the chain-mint + marshal + sink-write sequence
	// executes under the same lock so events land on disk in the same
	// order they were chained. See #212 / governance R5.
	lastHash string
...
func (a *AuditLogger) Emit(event AuditEvent) {
	...
	// Hash-chain stamp (#212, governance R5). Chain-set + marshal +
	// tail-hash-update + sink-write all happen under the SAME mutex —
	// two concurrent emits must not race on a.lastHash, and the sink
	// writes MUST land in the same order they were chained (otherwise
	// verifiers see events referencing a prev_hash whose event hasn't
	// arrived yet). The mutex serializes both the chain-mint and the
	// downstream I/O.
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	// Tail hash is sha256 of the canonical JSON bytes WITHOUT the
	// trailing newline — the verifier recomputes over the same
	// canonical form, so any tampering (added/removed byte, altered
	// field) makes the next event's prev_hash unreconcilable.
