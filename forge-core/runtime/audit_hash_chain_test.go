...
func TestHashChain_VerifyWalksCleanly(t *testing.T) {
	...
	res, err := VerifyAuditLog(&buf, VerifyOptions{})
	if err != nil {
		t.Fatalf("VerifyAuditLog: %v", err)
	}
	if !res.OK() {
		t.Fatalf("verify failed at line %d (expected %q, got %q)",
			res.FirstBadLine, res.BadEvent.PrevHash, res.BadEvent.PrevHash)
	}
...
func TestHashChain_TamperingDetected(t *testing.T) {
	...
	res, err := VerifyAuditLog(strings.NewReader(tampered), VerifyOptions{})
	if err != nil {
		t.Fatalf("VerifyAuditLog: %v", err)
	}
	if res.OK() {
	...
	if res.FirstBadLine != 3 {
		...
		t.Errorf("tampering detected at line %d, want line 3", res.FirstBadLine)
	}
}
...
func TestHashChain_DeletionDetected(t *testing.T) {
	...
	// Drop line 2.
	trimmed := lines[0] + "\n" + lines[2] + "\n"
	res, err := VerifyAuditLog(strings.NewReader(trimmed), VerifyOptions{})
	if err != nil {
		t.Fatalf("VerifyAuditLog: %v", err)
	}
	if res.OK() {
	...
	if res.FirstBadLine != 2 {
		t.Errorf("deletion detected at line %d, want line 2", res.FirstBadLine)
	}
}
...
func TestHashChain_ConcurrentEmitsProduceValidChain(t *testing.T) {
	...
	if !res.OK() {
		t.Fatalf("chain broke under concurrent emits at line %d", res.FirstBadLine)
	}
	if res.EventCount != N {
...
func TestHashChain_VerifyDetectsMalformedLine(t *testing.T) {
	...
	if res.FirstBadLine != 2 {
		t.Errorf("malformed line reported at %d, want 2", res.FirstBadLine)
	}
}
